...
namespace Prototype
{
    class ChocolateBarFood : Food
    {

        SoundEffect soundEffectthree;
        SoundEffectInstance soundEffectIntancethree;

        public ChocolateBarFood(FoodManager fm, Player playership, Vector2 initiallocation, Vector2 direction, float baserotation, float rotationrate, Game game, List<Food> shotlist)
            : base(fm, playership, initiallocation, direction, baserotation, rotationrate, game, shotlist)
        {
            FoodID = (int)FoodTypes.ChocolateBar;

            spriteTexture = fm.ChocolateBarTexture;
    ...
    new Color[this.spriteTexture.Width * this.spriteTexture.Height];
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            Rectangle FoodRect = new Rectangle((int)Location.X - (GameFoodManager.ChocolateBarTexture.Width / 2), (int)Location.Y - (GameFoodManager.ChocolateBarTexture.Width / 2), GameFoodManager.ChocolateBarTexture.Width, GameFoodManager.ChocolateBarTexture.Width);
